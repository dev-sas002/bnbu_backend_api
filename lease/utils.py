...
def analyze_document_with_gpt(document):
    ...
    try:
        with open(document.file.path, 'rb') as file:
            ...
            chunk_size = 5
            chunks = [
                ''.join(reader.pages[j].extract_text() for j in range(i, min(i + chunk_size, num_pages)))
        ...
        created_time = datetime.fromtimestamp(final_analysis_response['created'], tz=timezone.utc).isoformat()
        ...
        gpt_message = message_lines[1] if len(message_lines) > 1 else ""  # The message after the first newline
